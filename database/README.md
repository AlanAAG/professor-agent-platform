...
| `embedding` | `VECTOR(768)` or `VECTOR(1024)` | The vector representation of `content` generated by the configured embedding model. Use `VECTOR(768)` for Gemini (`text-embedding-004` / `models/embedding-001`) or `VECTOR(1024)` for Mistral (`mistral-embed`). |
